...
import { createTokenHolderTx, createUsageFeeTx, createDataTx, getTxFee } from './fees';
...
export const prepOrder = async ({
	...
	const arBalance = parseFloat(client.ar.winstonToAr(await client.wallets.getBalance(addr)));
	...
	const [tokenHolderPortionTx, usageFeeTx] = await Promise.all([
		tokenHolderPortionTxPromise,
		usageFeeTxPromise
	]);
	...
	console.log(
		{ txFees, usageFee, tokenHolderFee },
		{ dataReward: parseFloat(client.ar.winstonToAr(dataTx.reward)) }
	);
